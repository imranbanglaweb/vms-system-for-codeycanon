...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom">
        <h3 class="mb-0 fw-bold text-dark">
            <i class="fa-solid fa-clipboard-check me-2 text-primary"></i> Transport Approval: <span class="text-primary">{{ $requisition->requisition_no }}
        </h3>
        <a href="{{ route('maintenance_transport_approvals.index') }}" class="btn btn-secondary btn-sm">
        ...
        <div class="col-md-4">
            <div class="card shadow-sm border-0 mb-3">
                <div class="card-header bg-dark text-white d-flex align-items-center">
                    ...
                    <h5 class="mb-0">Transport Approval</h5>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                        <label class="form-label fw-bold">Department Status</label>
                        <div class="d-block">
                            <span class="badge bg-{{ $requisition->department_status == 'Approved' ? 'success' : 'warning' }} badge-lg">{{ $requisition->department_status ?? 'Pending' }}</span>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-bold">Transport Status</label>
                        <div class="d-block">
                            <span class="badge bg-info badge-lg">{{ $requisition->transport_status ?? 'Pending' }}</span>
                        </div>
                    </div>
                    <hr>
                    @if(in_array($requisition->transport_status, ['Pending', null]) && $requisition->department_status == 'Approved')
                    <div class="d-grid gap-2">
                        <button type="button" class="btn btn-success btn-sm fw-bold" onclick="approveRequisition({{ $requisition->id }})">
                            <i class="fa fa-check-circle me-2"></i> Approve Transport
                        </button>
                        <button type="button" class="btn btn-danger btn-sm fw-bold" onclick="rejectRequisition({{ $requisition->id }})">
                            <i class="fa fa-times-circle me-2"></i> Reject Transport
                        </button>
                    </div>
                    @elseif($requisition->transport_status == 'Approved')
                    <div class="alert alert-success">
                        <i class="fa fa-check-circle me-1"></i> Transport approved.
                        @if($requisition->transport_approved_by)
                            <br><small class="text-muted">By: {{ $requisition->transportApprovedBy->name ?? 'Transport' }}</small>
                        @endif
                        @if($requisition->transport_approved_at)
                            <br><small class="text-muted">On: {{ date('d M, Y H:i', strtotime($requisition->transport_approved_at)) }}</small>
                        @endif
                    </div>
                    @elseif($requisition->transport_status == 'Rejected')
                    <div class="alert alert-danger">
                        <i class="fa fa-times-circle me-1"></i> Transport rejected.
                    </div>
                    @endif
                </div>
            </div>
...
<script>
var maintenanceTransportApprovalBaseUrl = '{{ route('maintenance_transport_approvals.index') }}';
$(document).ready(function() {
    ...
    $('#approveForm').off('submit').on('submit', function(e) {
        ...
        Swal.fire({
            title: 'Are you sure?',
            text: 'You want to approve transport for this maintenance requisition?',
            icon: 'question',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('maintenance_transport_approvals.approve', $requisition->id) }}",
                    method: 'POST',
                    data: $(this).serialize(),
    ...
    $('#rejectForm').off('submit').on('submit', function(e) {
        ...
        Swal.fire({
            title: 'Are you sure?',
            text: 'You want to reject transport for this maintenance requisition?',
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('maintenance_transport_approvals.reject', $requisition->id) }}",
                    method: 'POST',
                    data: $(this).serialize(),
                    success: function(response) {
                        if (response.status === 'success') {
...
function approveRequisition(id) {
    Swal.fire({
        ...
        text: 'Are you sure you want to approve transport for this maintenance requisition?',
        icon: 'question',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/admin/dashboard/approvals/maintenance-transport/' + id + '/approve',
                method: 'POST',
...
// Global function for reject with SweetAlert
