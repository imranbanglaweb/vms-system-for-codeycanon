...
if(nextli.data('transition')=="fade"){nextli.css({'opacity':1});opt.slots=1;prepareOneSlide(nextsh,opt,true);nextsh.find('.defaultimg').css({'opacity':0});var ssamount=0;nextsh.find('.slotslide').each(function(i){var ss=$(this);ssamount++;ss.css({'opacity':0});ss.animate({'opacity':1},{duration:500,queue:false});});setTimeout(function(){opt.transition=0;opt.act=opt.next;moveSelectedThumb(container);},300);}
...
nextcaption.data('timer',setTimeout(function(){if(nextcaption.hasClass('fade')){nextcaption.animate({'opacity':1});}
...
opt.cdint=setInterval(function(){if(opt.conthover!=1)opt.cd=opt.cd+100;if(opt.cd>=opt.delay){opt.cd=0;opt.act=opt.next;opt.next=opt.next+1;if(opt.next>container.find('>ul >li').length-1)opt.next=0;swapSlide(container,opt);if(bt.length>0){bt.css({'width':'0%'});bt.animate({'width':"100%"},{duration:(opt.delay-100),queue:false,easing:"linear"});}}},100);container.hover(function(){if(opt.onHoverStop=="on"){opt.conthover=1;bt.stop();}},function(){if(opt.onHoverStop=="on"){opt.conthover=0;bt.animate({'width':"100%"},{duration:((opt.delay-opt.cd)-100),queue:false,easing:"linear"});}});}})(jQuery);