...
class DriverController extends Controller
{
    ...
    public function driverDashboard()
    {
        ...
        if (request()->expectsJson() || request()->is('api/*')) {
            ...
            // Get last used vehicle for API response
            $lastVehicleData = null;
            if ($activeTrip && $activeTrip->assignedVehicle) {
                $lastVehicleData = [
                    'id' => $activeTrip->assignedVehicle->id,
                    'vehicleName' => $activeTrip->assignedVehicle->vehicle_name,
                    'numberPlate' => $activeTrip->assignedVehicle->number_plate,
                    'capacity' => $activeTrip->assignedVehicle->capacity,
                    'type' => $activeTrip->assignedVehicle->vehicleType->name ?? null,
                ];
            } else {
                $lastTrip = \App\Models\Requisition::where('assigned_driver_id', $driver->id)
                    ->whereNotNull('assigned_vehicle_id')
                    ->orderBy('travel_date', 'desc')
                    ->orderBy('id', 'desc')
                    ->with(['assignedVehicle'])
                    ->first();
                if ($lastTrip && $lastTrip->assignedVehicle) {
                    $lastVehicleData = [
                        'id' => $lastTrip->assignedVehicle->id,
                        'vehicleName' => $lastTrip->assignedVehicle->vehicle_name,
                        'numberPlate' => $lastTrip->assignedVehicle->number_plate,
                        'capacity' => $lastTrip->assignedVehicle->capacity,
                        'type' => $lastTrip->assignedVehicle->vehicleType->name ?? null,
                    ];
                }
            }
        ...
        // Get last used vehicle from active or most recent trip (to show on dashboard after trip ends)
        $lastVehicle = null;
        if ($activeTrip && $activeTrip->assignedVehicle) {
            $lastVehicle = $activeTrip->assignedVehicle;
        } else {
            $lastTrip = \App\Models\Requisition::where('assigned_driver_id', $driver->id)
                ->whereNotNull('assigned_vehicle_id')
                ->orderBy('travel_date', 'desc')
                ...
                ->first();
            if ($lastTrip) {
                $lastVehicle = $lastTrip->assignedVehicle;
            }
        }
        ...
        return view('admin.dashboard.driver.dashboard', compact('driver', 'todayTrips', 'upcomingTrips', 'recentTrips', 'assignedTrips', 'activeTrip', 'pendingTripsCount', 'activeTripsCount', 'completedTripsCount', 'pendingTrips', 'lastVehicle'));
    }
    ...
    /**
     * Driver Live Tracking - shows real-time map tracking during trip
     */
    public function driverLiveTracking()
    {
        $driver = $this->getDriverByUser();
        $activeTrip = null;
        ...
        if ($driver) {
            $activeTrip = \App\Models\Requisition::where('assigned_driver_id', $driver->id)
                ->where('transport_status', 'In Transit')
                ->with(['vehicle', 'assignedVehicle', 'passengers'])
                ->first();
        }

        return view('admin.dashboard.driver.live-tracking', compact('driver', 'activeTrip'));
    }
    ...
    public function driverTripStatus($id = null)
    {
        ...
        $activeTrips = \App\Models\Requisition::where(function ($query) use ($driver) {
            ...
            ->with(['vehicle', 'employee', 'assignedVehicle', 'passengers'])
            ->orderBy('travel_date', 'desc')
            ->get();
